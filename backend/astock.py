...
def all_sectors() -> dict:
    """全部行业板块 + 全部概念板块（板块级、零个股名单）。
    
    行业板块：来自新浪财经（84 个行业，带实时涨跌/涨跌家数）。
    概念板块：来自同花顺（375 个概念，名称+代码；暂无实时行情）。
    缓存 10 分钟。"""
    industries = _sina_industries()
    concepts = _ths_concepts()
    return {"industries": industries, "concepts": concepts}


def _ths_concepts() -> list[dict]:
    """同花顺概念板块名称列表（375 个概念，仅名称+代码）。"""
    try:
        ak = _akshare()
...
def _sina_industries() -> list[dict]:
    """新浪财经行业板块列表（84 个行业，带实时涨跌/涨跌家数）。"""
    import re as _re
    ...
    try:
        ...
        r = s.get("https://money.finance.sina.com.cn/q/view/newFLJK.php?param=hy", timeout=10)
        r.encoding = "gbk"
        text = r.text
    except Exception:
        return []
    ...
    for _key, val in data.items():
        ...
        rows.append({
            "code": _key,
            "name": name,
            "change_pct": round(float(parts[3] or 0), 2),
            "up_count": int(parts[2] or 0),
            "down_count": 0,
            "kind": "行业",
        })
    return rows
...
# 模块级缓存，供 get_sector_detail 复用，避免每次查详情都重拉全量列表。
_ALL_SECTORS_CACHE: dict = {"industries": [], "concepts": []}
_ALL_SECTORS_TS = 0.0


def get_sector_detail(kind: str, code: str) -> dict | None:
    """从缓存的板块全量列表中按 kind + code 取单个板块详情。"""
    global _ALL_SECTORS_TS, _ALL_SECTORS_CACHE
    if time.time() - _ALL_SECTORS_TS < 600 and _ALL_SECTORS_CACHE.get("industries"):
        data = _ALL_SECTORS_CACHE
    else:
        data = all_sectors()
        _ALL_SECTORS_CACHE = data
        _ALL_SECTORS_TS = time.time()
    for sector in data.get("industries", []) + data.get("concepts", []):
        if sector.get("kind") == kind and sector.get("code") == code:
            return sector
    return None


_TEAJOIN_STOCKS: tuple[float, list[dict]] | None = None
_TEAJOIN_MEMBERS: dict[tuple[str, str], tuple[float, list[dict]]] = {}
_TEAJOIN_SECTORS: tuple[float, dict] | None = None


def _teajoin_code(value: object) -> str:
...
def _normalized_daily_sector(row: dict, kind: str, trade_date: str, source_api: str) -> dict | None:
    ...
    name = str((row.get("industry") if kind == "行业" else row.get("name")) or "").strip()
    close = _number(row.get("close") if kind == "行业" else row.get("close_price"))
    pct_change = _number(row.get("pct_change"))
    net_amount = _number(row.get("net_amount"))
    member_count = _integer(row.get("company_num"))
    lead_stock = str(row.get("lead_stock") or "").strip()
    if (
        str(row.get("trade_date") or "") != trade_date
        or not code or not name or close is None or pct_change is None or net_amount is None
        or member_count < 1 or not lead_stock
    ):
        return None
    return {
        "kind": kind, "code": code, "name": name, "as_of": trade_date,
        "close": close, "pct_change": pct_change, "member_count": member_count,
        "lead_stock": lead_stock, "net_amount": net_amount,
        "source": "TeaJoin/Tushare", "source_api": source_api,
        "market": "CN-A", "currency": "CNY", "timezone": "Asia/Shanghai", "frequency": "1d",
        "data_status": "complete",
    }


def _verified_ths_members(code: str, trade_date: str) -> list[dict]:
    rows = None
    for attempt in range(2):
        try:
            rows = teajoin.call("ths_member", {"ts_code": code}, "ts_code,con_code,con_name,in_date,out_date")
            break
        except teajoin.TeaJoinUpstreamError as exc:
            if attempt or exc.__cause__ is None:
                raise
            time.sleep(0.5)
    if rows is None:
        raise teajoin.TeaJoinUpstreamError("TeaJoin 成分股请求未返回数据")
    result: list[dict] = []
    seen: set[str] = set()
    for row in rows:
        member_code = _teajoin_code(row.get("con_code"))
        name = str(row.get("con_name") or "").strip()
        out_date = str(row.get("out_date") or "")
        if not member_code or len(member_code) != 6 or not member_code.isdigit() or not name:
            continue
        if out_date and out_date <= trade_date:
            continue
        if member_code in seen:
            continue
        seen.add(member_code)
        result.append({
            "code": member_code, "name": name, "market": "A股",
            "joined_at": str(row.get("in_date") or ""),
        })
    return result
...
def build_verified_sector_snapshot(trade_date: str) -> tuple[dict, dict[tuple[str, str], list[dict]]]:
    ...
    for kind, source_api, rows in (
        ("行业", "moneyflow_ind_ths", industry_rows),
        ("概念", "moneyflow_cnt_ths", concept_rows),
    ):
        for raw in rows:
            normalized = _normalized_daily_sector(raw, kind, trade_date, source_api)
            if normalized is None:
                excluded["incomplete_daily_fields"] = excluded.get("incomplete_daily_fields", 0) + 1
            else:
    ...
    if completeness["candidate_count"] != completeness["published_count"] + completeness["excluded_count"]:
        raise RuntimeError("sector snapshot completeness accounting failed")
    return {
        "snapshot_id": snapshot_id, "status": "completed", "as_of": trade_date,
        "retrieved_at": retrieved_at, "source": "TeaJoin/Tushare",
        "market": "CN-A", "currency": "CNY", "timezone": "Asia/Shanghai", "frequency": "1d",
        "method_version": _SECTOR_METHOD_VERSION, "completeness": completeness, "sectors": public_rows,
    }, members


def _teajoin_latest_industry_daily() -> tuple[str, list[dict]]:
    """取可获得的最近交易日行业日线，避免把未入库的当日数据误报为零。"""
    today = datetime.now().date()
    calendar_rows = teajoin.call(
        "trade_cal",
        {
            "exchange": "SSE",
    ...
    trade_dates = sorted(
        {
            ...
            if str(row.get("is_open")) == "1" and str(row.get("cal_date") or "")
        },
        reverse=True,
    )
    if not trade_dates:
        raise teajoin.TeaJoinUpstreamError("TeaJoin 未返回最近交易日历")

    for trade_date in trade_dates:
        rows = teajoin.call(
            "moneyflow_ind_ths",
            {"trade_date": trade_date},
            "trade_date,ts_code,industry,lead_stock,close,pct_change,company_num,net_amount",
        )
        valid_rows = [
            row for row in rows
            if str(row.get("ts_code") or "").strip() and str(row.get("industry") or "").strip()
        ]
        if valid_rows:
            return trade_date, valid_rows
    raise teajoin.TeaJoinUpstreamError("TeaJoin 最近交易日未返回行业日线数据")


def teajoin_all_sectors() -> dict:
    """行业采用最新可得日线，概念保留同花顺主数据，成员均走同一代码体系。"""
    global _TEAJOIN_SECTORS
    if _TEAJOIN_SECTORS and time.time() - _TEAJOIN_SECTORS[0] < 600:
        return _TEAJOIN_SECTORS[1]
    as_of, industry_rows = _teajoin_latest_industry_daily()
    data = {
        "industries": [], "concepts": [],
        "source": "TeaJoin/Tushare moneyflow_ind_ths + ths_index",
        "as_of": as_of,
    }
    for row in industry_rows:
        code, name = str(row.get("ts_code") or ""), str(row.get("industry") or "")
        if not code or not name:
            continue
        data["industries"].append({
            "code": code, "name": name, "kind": "行业",
            "member_count": _integer(row.get("company_num")),
            "change_pct": _number(row.get("pct_change")), "up_count": None, "down_count": None,
            "lead_stock": str(row.get("lead_stock") or ""), "net_amount": _number(row.get("net_amount")),
            "as_of": as_of,
        })

    for row in teajoin.call("ths_index"):
        if str(row.get("type") or "") != "N":
            continue
        code, name = str(row.get("ts_code") or ""), str(row.get("name") or "")
        member_count = _integer(row.get("count"))
        if not code or not name or member_count < 2:
            continue
        data["concepts"].append({
            "code": code, "name": name, "kind": "概念",
            "member_count": member_count,
            "change_pct": None, "up_count": None, "down_count": None,
            "lead_stock": "", "net_amount": None,
            "as_of": None,
        })
    for key in ("industries", "concepts"):
        data[key].sort(key=lambda row: row["code"])
    _TEAJOIN_SECTORS = (time.time(), data)
    return data


def teajoin_sector_detail(kind: str, code: str) -> dict | None:
    data = teajoin_all_sectors()
    bucket = "industries" if kind == "行业" else "concepts" if kind == "概念" else None
    if not bucket:
        return None
    return next((row for row in data[bucket] if row["code"] == code), None)


def teajoin_sector_members(kind: str, code: str) -> list[dict]:
    """TeaJoin 的同花顺板块成分；上游故障不能伪装为无数据。"""
    cache_key = (kind, code)
    hit = _TEAJOIN_MEMBERS.get(cache_key)
    if hit and time.time() - hit[0] < 300:
        return hit[1]
    if teajoin_sector_detail(kind, code) is None:
        raise ValueError("板块代码与分类不匹配或板块不存在")
    rows = teajoin.call("ths_member", {"ts_code": code}, "ts_code,con_code,con_name,in_date,out_date")
    result = [
        {"code": _teajoin_code(row.get("con_code")), "name": str(row.get("con_name") or ""),
         "market": "A股", "joined_at": str(row.get("in_date") or "")}
        for row in rows
        if _teajoin_code(row.get("con_code")) and row.get("con_name")
    ]
    _TEAJOIN_MEMBERS[cache_key] = (time.time(), result)
    return result


def _teajoin_stock_list() -> list[dict]:
    global _TEAJOIN_STOCKS
    if _TEAJOIN_STOCKS and time.time() - _TEAJOIN_STOCKS[0] < 600:
        return _TEAJOIN_STOCKS[1]
    rows = teajoin.call("stock_basic", {"list_status": "L"}, "ts_code,symbol,name,market,industry,list_date")
    result = [
        {"code": _teajoin_code(row.get("ts_code") or row.get("symbol")), "ts_code": str(row.get("ts_code") or ""),
         "name": str(row.get("name") or ""), "market": str(row.get("market") or ""), "industry": str(row.get("industry") or "")}
        for row in rows
        if _teajoin_code(row.get("ts_code") or row.get("symbol")) and row.get("name")
    ]
    _TEAJOIN_STOCKS = (time.time(), result)
    return result


def teajoin_stock_search(query: str, limit: int = 20) -> list[dict]:
    """按证券代码、交易所后缀代码或证券名称检索真实 A 股主数据。"""
    needle = query.strip().lower()
    return [row for row in _teajoin_stock_list() if needle in row["code"].lower()
            or needle in row["ts_code"].lower() or needle in row["name"].lower()][:limit]
