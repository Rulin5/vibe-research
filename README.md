...
> **Vibe-Research：个人 AI 投研系统** · 覆盖 A 股 / 美股 / 港股的个人投研工具。
>
...
> *Vibe-Research 是开源投研看板：连接公开市场数据，也可由你选择接入自己的 AI / Agent；它不推荐个股。*
...
| 🔌&nbsp;**AI&#8288;设&#8288;置** | 本机 CLI 接入（无需填写模型服务密钥）· 多模型接口（自动填写接口地址）· MCP（挂进 Claude Code 等 Agent）|
...
| 数据类别 | 具体来源 | 用途 | 认证要求 |
|---|---|---|---|
| A 股实时行情 | 腾讯财经 `qt.gtimg.cn` | 实时价格、涨跌、PE/PB、市值 | 无需登录或密钥 |
| A 股研报、公告、资金与海外行情 | 东方财富 `reportapi`、`push2`、`datacenter` 等公开接口 | 研报、公告、资金流、板块、全球指数及美港韩股行情 | 无需登录或密钥 |
| A 股财务、新闻、估值历史分位与市场情绪 | AkShare 对接的公开页面与接口（含同花顺、百度股市通、巨潮） | 财务摘要、新闻、公告、估值分位、市场情绪 | 无需登录或密钥；上游可用性会变化 |
| K 线与备用财务数据 | 通达信行情服务（Mootdx） | K 线、F10 和备用财务数据 | 无需登录或密钥 |
| 产业与市场资讯 | `backend/news_sources.json` 内的公开 RSS（当前配置 12 个赛道、108 个源） | 资讯雷达 | 无需登录或密钥 |

数据源均是外部公开接口，可能出现限流、字段变更或临时不可用。后端会返回具体错误或 501 依赖提示，不会以零值替代。
...
首次运行会在本机终端询问 TeaJoin 和可选的 StepFun API Key，并自动生成数据库密码、Session 密钥和凭据加密密钥。启动完成后访问 <http://127.0.0.1:5900>。

真实密钥只保存在本机 `deploy/demo.env`。**不要把 deploy/demo.env 上传到 GitHub**；更换使用者时重新运行 `.\start-demo.ps1 -ResetConfig`。停止服务使用 `docker compose -f compose.demo.yaml down`，连同演示数据彻底删除使用 `docker compose -f compose.demo.yaml down --volumes`。

### 手动开发运行

```powershell
# 后端（:8900，Windows PowerShell）
cd backend
py -3 -m venv .venv
.\.venv\Scripts\python.exe -m pip install -r requirements.txt -r requirements-dev.txt
.\.venv\Scripts\python.exe -m uvicorn app:app --host 127.0.0.1 --port 8900

# 板块中心：触发同交易日行业/概念验证快照刷新（后台执行）
# POST http://127.0.0.1:8900/api/sectors/refresh
# GET  http://127.0.0.1:8900/api/sectors/status
# 刷新完成前 /api/all-sectors 返回 503，不混用静态分类或展示空成分股。

# 新开一个 PowerShell 窗口运行前端（:5899）
cd frontend
npm ci
npm run dev -- --host 127.0.0.1
# 浏览器打开 http://127.0.0.1:5899
```

公共数据、持仓和研报管理不需要账户、密码、登录态、后端访问密钥或第三方 API Key。AI 是可选增强功能，只有主动点击“问 AI / 复盘 / 辩论”时才需要你自行配置模型服务。

## 接入 AI

在「接入 AI」页配置一次，全站的「问 AI / 复盘 / 今日要点」就都用你自己的模型。**分析都由你的模型给出，本产品不校准、无倾向。** 三种方式：

### 1. 订阅接入（调用本机 CLI，无需填写模型服务密钥）

用你自己的**订阅额度**，不用付 API 费。已支持：**Claude Code · Codex · Qwen Code · DeepSeek CLI**。

- **前提**：① 后端跑在你本机（云端读不到你本机 CLI）；② 对应 CLI 已安装并处于可用状态，命令在 `PATH` 上。例如：
...
「AI 设置 → API 接入」选一个模型，接口地址自动填好，再填写模型服务密钥。内置 **DeepSeek / 豆包 / MiniMax / OpenAI / OpenRouter / Groq / Together / MiMo / 任意 OpenAI 兼容端点**。这条支持函数调用——AI 会自己调数据工具（行情/估值/研报/新闻）再作答。模型服务密钥只存本地浏览器、随请求发给你自己的后端、不上传、不进仓库。
...
2. **使用「订阅接入」（本机 CLI）以替代模型服务密钥**。若选择的第三方 CLI 自身要求账户或订阅，该要求由其提供方决定，并非本项目的访问门槛。
...
- **持仓 / 关注股 / 上传的研报 / 模型服务密钥只存本地，不上传、不进仓库。**
