  ...
  return (
    <div>
      ...
      <div className="mb-5 flex gap-2">
        <StockSearchInput value={code} onChange={setCode} allowGlobalCode className="w-80" placeholder="股票名称或代码；也可输入 AAPL / 00700" />
        <button
          onClick={run}
          disabled={loading}
      ...
      {!val && !err && !loading && (
        <GlassCard>
          <div className="py-10 text-center text-sm text-muted-foreground">
            ...
            <span className="text-xs text-muted-foreground/60">数据来自公开源（腾讯行情 / 东财研报 / akshare）；清数智算 不预置任何标的、不做推荐。</span>
          </div>
        </GlassCard>
      )}
