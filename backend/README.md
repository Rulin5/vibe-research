...
本服务只支持 `127.0.0.1` 本机回环地址运行。公共数据 API、持仓和研报管理不要求账户、密码、登录态、后端访问密钥或第三方 API Key；AI 端点仅在用户主动传入自己的模型配置时调用外部模型服务。

| 端点 | 说明 | 依赖 |
|---|---|---|
| `GET /api/health` | 健康检查 | — |
| `GET /api/indices` | 大盘指数实时行情 | stdlib |
| `GET /api/market/index-candles?symbols=CN.SH.000001,GLOBAL.SPX&period=1d&limit=60` | TeaJoin 真实指数日K（含来源、交易日、时区和市场状态） | TeaJoin |
...
```
`llm` 由前端从本地配置随请求带上，后端不持久化模型服务密钥。
...
用你自己的订阅额度调数据、多步分析——无需填写模型服务密钥、不占本产品成本。
