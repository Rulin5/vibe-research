...
def test_debate_route_validation(body, code, authenticated_client, monkeypatch):
    private_client, headers = authenticated_client
    monkeypatch.delenv("VR_AI_STEPFUN_API_KEY", raising=False)
    response = private_client.post("/api/debate", json=body, headers=headers)
    assert response.status_code == (code if body["code"] == "abc" else 422)


def test_reflect_route_rejects_empty_source(authenticated_client):
    private_client, headers = authenticated_client
    assert private_client.post("/api/reflect", json={"source": "  "}, headers=headers).status_code == 400


def test_reflect_route_requires_llm_config(authenticated_client, monkeypatch):
    private_client, headers = authenticated_client
    monkeypatch.delenv("VR_AI_STEPFUN_API_KEY", raising=False)
    r = client.post("/api/reflect", json={"source": "一段分析", "llm": {**_LLM, "baseURL": ""}})
    r = private_client.post("/api/reflect", json={"source": "一次分析"}, headers=headers)
