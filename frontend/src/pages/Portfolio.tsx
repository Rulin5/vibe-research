...
const fmtPx = (v: number | null | undefined) => v == null ? "—" : v.toLocaleString("zh-CN", { maximumFractionDigits: 4 });
...
export function Portfolio() {
  ...
  const remove = async (id: string) => {
    try { await api.removeHolding(id); await load(); } catch { /* keep last confirmed data */ }
  };
  ...
  const removeClosed = async (id: string) => {
    try { await api.removeClosed(id); await load(); } catch { /* keep last confirmed data */ }
  };
    ...
    <div>
      ...
      {totals && holdings.length > 0 && (
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            { k: "浮动盈亏", v: (totals.pnl != null && totals.pnl > 0 ? "+" : "") + fmt(totals.pnl), c: pnlColor(totals.pnl) },
            { k: "盈亏比例", v: totals.pnl_pct == null ? "—" : (totals.pnl_pct > 0 ? "+" : "") + totals.pnl_pct + "%", c: pnlColor(totals.pnl) },
          ].map((m) => (
      ...
      <GlassCard className="mb-4">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <StockSearchInput value={code} onChange={setCode} className="w-56" />
          </div>
          <div>
              ...
              <tbody>
                {holdings.map((h) => (
                  <tr key={h.id} className="border-b border-border/30">
                    ...
                    <td className="px-2 py-2.5">
                      <button onClick={() => remove(h.id)} className="text-muted-foreground/50 hover:text-destructive" title="删除">
      ...
      <GlassCard className="mb-4 mt-6">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">股票名称或代码</label>
            <StockSearchInput value={cCode} onChange={setCCode} className="w-56" />
          </div>
          <div>
