...
SYSTEM_PROMPT = f"""你是 清数智算 里的投研助理。你可以调用工具获取客观数据来支撑回答，A 股工具一律传 6 位代码：
...
# 本项目只支持本机回环地址运行，因此允许用户接入本机 Ollama 等兼容服务；云元数据网段始终禁止。
...
def _ip_blocked(host: str) -> bool:
    ...
    if any(ip in n for n in _METADATA_NETS):  # 云元数据 / 链路本地：SSRF 头号目标，始终禁
        return True
    if is_public_mode() and any(ip in n for n in _PRIVATE_NETS):  # 公网姿态再禁内网 / 本机
        return True
    return False
...
def _check_base_url(url: str) -> None:
    ...
    本机模式放行 127.0.0.1 等本机地址（方便接本机 Ollama / 网关），只挡 169.254 元数据。"""
    p = urlparse(url or "")
    ...
    if is_public_mode():
        if p.scheme != "https":
            raise RuntimeError("Base URL must use https in public mode")
        allowed_hosts = ai_allowed_hosts()
        if host.lower().rstrip(".") not in allowed_hosts:
            raise RuntimeError("Base URL host is not allowed in public mode")
        # 公网姿态：域名也解析核对，防 DNS rebinding 指向内网
        try:
